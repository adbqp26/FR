...
async function loadGeothermalData() {
    try {
        const response = await fetch('Geothermal.json'); // Fetch the GeoJSON file
        
        if (!response.ok) {
            throw new Error('Failed to load geothermal data');
        }
        
        const geojsonData = await response.json(); // Parse JSON content
        console.log(geojsonData); // Use the GeoJSON data
        ...
        document.getElementById('loading').innerHTML = 
            'Error loading data. Please ensure Geothermal.json is in C:\\WebBased Map folder.';
    }
}
...
function populateFilters(geojsonData) {
    ...
    geojsonData.features.forEach(feature => {
        ...
        if (props.GEO_LIC) licenses.add(props.GEO_LIC);
    });
